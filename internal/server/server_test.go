...
func TestWatchParamSetsCookieAndRedirects(t *testing.T) {
	s := newTestServer(t)
	h := s.Handler()

	for _, q := range []string{"/?watch=1", "/?watch=true", "/?watch=yes"} {
		rec, cookies := doJSON(t, h, http.MethodGet, q, "", nil)
		if rec.Code != http.StatusSeeOther {
			t.Fatalf("%s: status = %d, want 303", q, rec.Code)
		}
		if loc := rec.Header().Get("Location"); loc != "/" {
			t.Fatalf("%s: Location = %q, want /", q, loc)
		}
		if v := cookieValue(cookies, watchCookie); v != "1" {
			t.Fatalf("%s: watch cookie = %q, want 1", q, v)
		}
	}
}

func TestWatchParamOffClearsCookie(t *testing.T) {
	s := newTestServer(t)
	h := s.Handler()
	cookie := &http.Cookie{Name: watchCookie, Value: "1", Path: "/"}

	for _, q := range []string{"/?watch=0", "/?watch=false", "/?watch=off"} {
		rec, cookies := doJSON(t, h, http.MethodGet, q, "", []*http.Cookie{cookie})
		if rec.Code != http.StatusSeeOther {
			t.Fatalf("%s: status = %d, want 303", q, rec.Code)
		}
		if v := cookieValue(cookies, watchCookie); v != "" {
			t.Fatalf("%s: watch cookie = %q, want cleared", q, v)
		}
		for _, c := range cookies {
			if c.Name == watchCookie && c.MaxAge != -1 {
				t.Fatalf("%s: cookie not expired: %+v", q, c)
			}
		}
	}
}

func TestWatchCookieRendersWatchClass(t *testing.T) {
	s := newTestServer(t)
	h := s.Handler()

	rec, _ := doJSON(t, h, http.MethodGet, "/", "", []*http.Cookie{{Name: watchCookie, Value: "1", Path: "/"}})
	if !strings.Contains(rec.Body.String(), `<html lang="en" class="watch">`) {
		t.Fatalf("expected watch class on <html>:\n%s", rec.Body)
	}

	rec, _ = doJSON(t, h, http.MethodGet, "/", "", nil)
	if strings.Contains(rec.Body.String(), `class="watch"`) {
		t.Fatalf("default page should not have watch class:\n%s", rec.Body)
	}
}

func TestWatchUAEnablesWatchClass(t *testing.T) {
	s := newTestServer(t)
	h := s.Handler()

	req := httptest.NewRequest(http.MethodGet, "/", nil)
	req.Header.Set("User-Agent", "Mozilla/5.0 (Watch; CPU OS 10_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/10.0 Safari/605.1.15")
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if !strings.Contains(rec.Body.String(), `class="watch"`) {
		t.Fatalf("expected watch class for Watch UA:\n%s", rec.Body)
	}

	req = httptest.NewRequest(http.MethodGet, "/", nil)
	req.Header.Set("User-Agent", "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Safari/605.1.15")
	rec = httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	if strings.Contains(rec.Body.String(), `class="watch"`) {
		t.Fatalf("phone UA should not render watch class:\n%s", rec.Body)
	}
}
